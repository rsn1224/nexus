...
/// count: ping 回数（推奨 10）
pub fn measure_network_quality(target: &str, count: u32) -> Result<NetworkQualitySnapshot, AppError> {
    // バリデーション
    ...
    info!("ネットワーク品質測定開始: target={}, count={}", target, count);
    ...
    let (avg_latency_ms, jitter_ms) = if rtt_values.is_empty() {
        ...
        let variance = rtt_values.iter()
            .map(|x| (x - avg).powi(2))
            .sum::<f64>() / rtt_values.len() as f64;
...
#[allow(dead_code)]
pub fn measure_network_quality_powershell(target: &str, count: u32) -> Result<NetworkQualitySnapshot, AppError> {
    validate_ping_target(target)?;
...
mod tests {
    ...
    fn test_parse_rtt_from_line() {
        // 標準形式
        assert_eq!(parse_rtt_from_line("Reply from 8.8.8.8: bytes=32 time=12ms TTL=116"), Some(12.0));
        assert_eq!(parse_rtt_from_line("Reply from 8.8.8.8: bytes=32 time=1ms TTL=116"), Some(1.0));
        ...
        assert_eq!(result, Some(0.5), "time<1ms should be parsed as 0.5, got {:?}", result);
