...
export function LogAnalysisPanel({ analysis }: LogAnalysisPanelProps): React.ReactElement {
  return (
    <div className="space-y-3">
      {/* サマリーセクション */}
      <div className="text-[11px] text-text-secondary font-semibold">▶ ANALYSIS SUMMARY</div>
      ...
      {/* 時間範囲 */}
      <div className="text-[10px] text-text-muted">RANGE: {analysis.timeRange}</div>

      {/* トップソース */}
      {analysis.topSources.length > 0 && (
        <div>
          <div className="text-[10px] text-text-muted mb-1">TOP SOURCES</div>
          <div className="space-y-0.5">
            {analysis.topSources.map(([source, count]) => (
              <div
                key={source}
                className="flex items-center justify-between text-[10px] text-text-secondary px-2 py-0.5 bg-base-800"
              >
