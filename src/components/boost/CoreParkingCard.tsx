    ...
    <Card title="CORE PARKING">
      <div className="flex flex-col gap-3 p-3">
        ...
        {coreParkingState === null ? (
          <div className="font-(--font-mono) text-[10px] text-text-muted">読み込み中...</div>
        ) : (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="font-(--font-mono) text-[11px] text-text-primary">{acPercent}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-(--font-mono) text-[10px] text-text-muted">現在の状態</span>
              <span
                className={`font-(--font-mono) text-[11px] font-semibold ${isParkingDisabled ? 'text-cyan-500' : 'text-text-muted'}`}
              >
