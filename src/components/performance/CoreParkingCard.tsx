        ...
        {coreParkingState === null ? (
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-text-muted">AC 電源時の最小コア率</span>
              <span className="font-mono text-xs text-text-primary">{acPercent}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-text-muted">現在の状態</span>
              <span
                className={`text-xs font-semibold ${isParkingDisabled ? 'text-accent-500' : 'text-text-muted'}`}
              >
