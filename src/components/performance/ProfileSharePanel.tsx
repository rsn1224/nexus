    ...
    <Modal isOpen title="EXPORT PROFILE" onClose={onClose}>
      <div className="flex flex-col gap-3">
        ...
        <textarea
          ...
          value={json}
          rows={12}
          className="w-full px-2 py-1 font-mono text-xs bg-base-700 border border-border-subtle text-text-secondary resize-none outline-none"
        />
        <div className="flex gap-2 justify-end">
          <button
            type="button"
            onClick={handleCopy}
            className={`px-4 py-[5px] text-xs border transition-colors ${
              copied
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-[5px] text-xs border border-border-subtle text-text-muted hover:bg-base-700 transition-colors"
          >
            CLOSE
          </button>
        </div>
    ...
    <Modal isOpen title="IMPORT PROFILE" onClose={onClose}>
      <div className="flex flex-col gap-3">
        ...
        <textarea
          ...
          className="w-full px-2 py-1 font-mono text-xs bg-base-700 border border-border-subtle text-text-secondary resize-none outline-none placeholder:text-text-muted"
        />
        {importError && <div className="text-xs text-danger-500">ERROR: {importError}</div>}
        <div className="flex gap-2 justify-end">
          <button
            type="button"
  ...
  return (
    <>
      <div className="flex items-center gap-2 pt-2 border-t border-border-subtle mt-1">
        <span className="text-xs text-text-muted flex-1">PROFILE SHARING</span>
        <button
          type="button"
          onClick={() => void handleExport()}
          disabled={!selectedProfile || isExporting}
          className={`px-3 py-[3px] text-xs border transition-colors ${
        ...
        <button
          ...
          className="px-3 py-[3px] text-xs border border-border-subtle text-text-secondary hover:border-accent-500 hover:text-accent-500 transition-colors"
        >
          IMPORT JSON
        </button>
      </div>
