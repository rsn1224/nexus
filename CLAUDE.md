...
（React 19 + Tauri v 2 + TypeScript + Zustand v 5 + Tailwind v 4 + Biome v 2 + Rust edition 2021）
...
│   ├── layout/TitleBar.tsx
│   └── ui/                   # Button, Card, Toggle, StatusBadge, SlidePanel...
├── stores/
...
| インラインスタイル禁止 | Tailwind v 4 className を使う |
...
| TS/TSX 200行制限 | 超える場合は分割 |
| Rust 300行制限 | 超える場合は分割 |
| ハードコード色禁止 | デザイントークン CSS 変数を使用 |
...
- 例外: `animate-spin` は border spinner パターン限定で許可（`border + border-t-transparent + rounded-full + animate-spin`）
...
| `notify_*` | **未実装** | ActionStrip から除外（非表示） |
...
| デザイン仕様 | `DESIGN.md` |
| ロードマップ | `ROADMAP.md` |
| Tauri v 2 注意点 | `.claude/rules/tauri-v2-gotchas.md` |
