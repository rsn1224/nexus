...
- フロントエンドは `null` の場合 `N/A` を `--color-text-muted` で表示する

---

## 14. プロセス保護

- boost.rs は `PROTECTED_PROCESSES: &[&str]` 定数を持つ
- KILL 前に名前チェックを行い、リスト内のプロセスはスキップする
- 結果は `action: "skipped_protected"` + `is_protected: true` で返す（エラーにしない）
- UI には `[PROT]` バッジ（fontSize: 9px、border: `--color-text-muted`）で表示する
- リアルタイムプロセスリスト UI は将来の Step（OH-B1）として保留

---

## 15. スタイル定数

- `src/lib/styles.ts` に共通スタイルオブジェクトを定義する
- **新規ファイルのみ** `S.xxx` を使用する
- 既存ファイル（HomeWing / BoostWing / LauncherWing / SettingsWing）のインライン書き換えは行わない
- 定義形式: `as const satisfies Record<string, React.CSSProperties>` で型安全にする
- 定数例: `S.monoLabel / S.monoValue / S.sectionTitle / S.microBadge`
- 新規ファイルは `src/lib/styles.ts` の `S.xxx` 定数を参照すること。
- 既存ファイルのインライン書き換えは ROI が低いため対象外。
